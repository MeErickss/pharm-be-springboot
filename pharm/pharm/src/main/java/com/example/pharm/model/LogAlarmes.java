...
public class LogAlarmes {
    ...
    @JoinColumn(name = "status_id", foreignKey = @ForeignKey(
            name = "fk_status_logAlarmes",
            foreignKeyDefinition = "FOREIGN KEY (status_descricao) REFERENCES STATUS(descricao) ON DELETE CASCADE"
    ))
    private Status status;

    public LogAlarmes() {} // Construtor vazio obrigatório para o JPA

    public LogAlarmes(Long id, Usuarios user, String descricao, String dataHora, Status status) {
        this.id = id;
        this.user = user;
